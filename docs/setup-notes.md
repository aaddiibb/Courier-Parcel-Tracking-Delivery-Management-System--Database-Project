...
## Day 9 — DBMS_OUTPUT Capture from Laravel

`DBMS_OUTPUT.PUT_LINE` writes to OCI8's internal buffer, which is not accessible via the standard PDO/yajra-oci8 interface. Two options were considered:

| Approach | Pro | Con |
|----------|-----|-----|
| Global Temporary Table (`plsql_log`) | Live data from the DB every request | Requires extra DDL; blocks must also INSERT |
| Hardcode expected output in controller | Zero extra infra | Output goes stale if seed data changes |

**Chosen: Global Temporary Table** (`plsql_log`).
...
- `ON COMMIT PRESERVE ROWS` — data survives COMMITs within a session; cleared only when the session ends.
- Each anonymous block deletes its own `block_id` rows, then re-inserts fresh lines, then COMMITs.
- The Laravel controller calls `DB::statement($plsql)` then `DB::select('SELECT message FROM plsql_log WHERE block_id = ? ORDER BY line_no', [$id])`.
- Exception blocks log inside the `EXCEPTION` handler so caught paths are always captured.

**Pre-requisite:** run `database/sql/05-plsql/00-logging-table.sql` via SQL*Plus before visiting `/lab/plsql`. The controller shows an error banner if the table is missing.
